"""
Standalone LBO Analysis for Canada Goose (GOOS)
...
def get_goos_financials():
    """Get GOOS financial data"""
    from goos_test_data import GOOS_FINANCIALS
    return GOOS_FINANCIALS
...
def main():
    """Run GOOS LBO analysis"""
    try:
        # Get financial data
        print("\nFetching GOOS financial data...")
        financials = get_goos_financials()
        
        print("\nCurrent Metrics:")
        print(f"Revenue: {format_currency(financials['revenue_ltm'])}")
        print(f"EBITDA: {format_currency(financials['ebitda_ltm'])}")
        print(f"Total Debt: {format_currency(financials['total_debt'])}")
        ...
        print("\nPurchase Summary:")
        print(f"Enterprise Value: {format_currency(output['purchase_price']['enterprise_value'])}")
        print(f"Equity Check: {format_currency(output['purchase_price']['sources_uses']['Sources']['Equity'])}")
        print(f"Total Debt: {format_currency(output['purchase_price']['sources_uses']['Sources']['Term Loan'] + output['purchase_price']['sources_uses']['Sources']['Revolver'])}")
        ...
        print("Year 0:")
        print(f"  Term Loan: {format_currency(output['debt_schedule'].loc[0, 'term_loan'])}")
        print(f"  Revolver: {format_currency(output['debt_schedule'].loc[0, 'revolver'])}")
        print(f"Year 5:")
        print(f"  Term Loan: {format_currency(output['debt_schedule'].loc[5, 'term_loan'])}")
        print(f"  Revolver: {format_currency(output['debt_schedule'].loc[5, 'revolver'])}")
        
        print("\nReturns:")
        print(f"Exit Enterprise Value: {format_currency(output['returns']['exit_enterprise_value'])}")
