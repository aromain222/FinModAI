#!/usr/bin/env python3
"""
Simple Unified DCF Model Builder
Defaults to Excel output, Google Sheets optional
"""
...
def write_to_excel(company, years, revenue, ebitda, ebit, fcf, terminal_value, enterprise_value, filename):
    ...
    if ws:
        ...
        ws.append(["FCF"] + [f"${val:,.0f}" for val in fcf])
        ws.append([])
...
def build_dcf_model(company_name, ticker=None):
    ...
    print(f"✅ Model successfully created in Excel!")

def main_menu():
    """Main interactive menu."""
    print("🚀 Simple DCF Model Builder")
    print("=" * 40)
    print("This tool creates DCF models with:")
    print("• Excel output")
