/**
 * Stock Move Explainer Component
 * 
 * Reusable component for analyzing stock price movements and catalysts
 * Used in both standalone stock pages and Market Intelligence Events & Movers tab
 */
...
type MarketMode = 'market' | 'ticker';

export interface StockMoveExplainerProps {
  defaultTicker?: string;
  defaultRange?: DateRange;
  marketMode?: MarketMode; // 'market' shows SPY/QQQ, 'ticker' allows search
  onMoveSelect?: (move: MoveEvent) => void; // Optional callback when move is selected
  compact?: boolean; // If true, shows simplified layout
  showDetailSheet?: boolean; // If true, shows detail in sheet instead of inline
}

// Skeleton loader
function MoveExplainerSkeleton({ compact }: { compact?: boolean }) {
  return (
    <div className="space-y-6">
      <Card className="border border-white/10 bg-black/60 p-6">
        <Skeleton className="h-8 w-48 mb-4" />
        <Skeleton className={cn('w-full', compact ? 'h-48' : 'h-64')} />
      </Card>
      {!compact && (
...
// Price Chart Component (same as existing)
function PriceChart({
  priceSeries,
  moves,
  selectedMove,
  ...
  return (
    <div className={cn('relative', shouldShowVolume ? (compact ? 'h-64' : 'h-96') : (compact ? 'h-56' : 'h-80'))}>
...
// Move Timeline Component (same as existing, but simplified for compact mode)
            ...
            {group.moves.map((moveWithCatalysts) => {
              ...
              return (
                <Card
                  key={move.date}
                  className={cn(
                    'border border-white/10 bg-black/60 p-3 cursor-pointer transition-all hover:bg-black/80',
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <span className={cn('font-medium text-white', compact ? 'text-xs' : 'text-sm')}>
                          {format(parseISO(move.date), compact ? 'MMM dd' : 'MMM dd, yyyy')}
                        </span>
                        <Badge
                          className={cn(
                            'text-xs',
                      ...
                      {!compact && (
                        <div className="flex items-center gap-2">
                          ...
                          {catalyst && (
                            <p className="text-xs text-slate-300 line-clamp-1 flex-1">{catalyst.label}</p>
                          )}
                        </div>
                      )}
                      {catalyst && compact && (
                        <p className="text-xs text-slate-400 line-clamp-1 mt-1">{catalyst.label}</p>
                      )}
                      {!compact && move.benchmarkReturnPct !== undefined && move.benchmarkTicker && (
                    ...
                    <ChevronRight className={cn('w-4 h-4 text-slate-400 flex-shrink-0 mt-1', isSelected && 'text-emerald-400')} />
                  </div>
                </Card>
              );
            })}
...
// Move Detail Component (exported for use in detail sheet)
export function MoveDetail({ moveWithCatalysts }: { moveWithCatalysts: MoveWithCatalysts | null }) {
  ...
  // If no AI explanation, show fallback
  if (!aiExplanation) {
    return (
      <Card className="border border-white/10 bg-black/60 p-6">
        ...
        {topCatalyst && topCatalyst.confidence !== 'low' ? (
          <>
            ...
            {topCatalyst.newsEvidence && topCatalyst.newsEvidence.length > 0 && (
              <div className="mb-6 rounded-lg border border-blue-500/20 bg-blue-500/5 p-5">
                ...
                <div className="space-y-3">
                  {topCatalyst.newsEvidence.slice(0, 3).map((ev, idx) => (
                    <a
                      key={idx}
                      href={ev.url}
                      target="_blank"
          ...
          <div className="mb-6 rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-5">
            <h3 className="text-sm font-semibold text-yellow-300 mb-3 uppercase tracking-wide">No dominant catalyst found</h3>
            <p className="text-sm text-slate-200 leading-relaxed mb-3">
              This move may reflect broader market or sector dynamics, or multiple smaller factors. No single dominant headline was identified.
            </p>
            {benchmarkComparison && (
        ...
        {benchmarkComparison && topCatalyst && (
...
// Ticker Search Component (simplified for reuse)
  ...
  return (
    <div className="relative">
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder={compact ? "Search ticker" : "Search ticker (e.g., AAPL, NVDA)"}
          value={value}
          onChange={(e) => {
          ...
          className={cn(
            'pl-10 pr-10 bg-black/40 border-white/10 text-white placeholder:text-slate-500',
            compact && 'h-9 text-sm'
          )}
        />
        {value && (
...
export default function StockMoveExplainer({
  defaultTicker = 'SPY',
  defaultRange = '3M',
  marketMode = 'ticker',
  onMoveSelect,
  ...
  const [explainOnlyTopN, setExplainOnlyTopN] = useState(true);
  const [marketTicker, setMarketTicker] = useState<'SPY' | 'QQQ'>('SPY');
  ...
  const handleSaveTicker = (t: string) => {
    const updated = [...savedTickers, t].slice(0, 10);
    setSavedTickers(updated);
    localStorage.setItem('savedTickers', JSON.stringify(updated));
  };

  // Use market ticker if market mode, otherwise use user-selected ticker
  const activeTicker = marketMode === 'market' ? marketTicker : ticker;

  // Date range based on selection
  const { start: startDate, end: endDate } = getDateRangeFromSelection(dateRange);

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['stock-moves', activeTicker, interval, dateRange, explainOnlyTopN, startDate.toISOString(), endDate.toISOString()],
    queryFn: async () => {
      if (!activeTicker) return null;
      const res = await fetch(
        `/api/stocks/catalysts?ticker=${activeTicker}&start=${startDate.toISOString().split('T')[0]}&end=${endDate.toISOString().split('T')[0]}&interval=${interval}&topN=10&includeAI=true&useStrictAI=true&includeBenchmark=true&explainOnlyTopN=${explainOnlyTopN}`
      );
  ...
  const handleMoveSelect = (move: MoveEvent) => {
  ...
  if (isLoading) {
    return <MoveExplainerSkeleton compact={compact} />;
  }

  if (isError || (!activeTicker && !isLoading)) {
    return (
      <div className={cn('text-center py-8', compact && 'py-4')}>
        <Card className="border border-white/10 bg-black/60 p-6">
          <AlertCircle className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-white mb-3">Unable to load stock data</h2>
  ...
  if (!data || data.moves.length === 0) {
    return (
      <div className={cn('text-center py-8', compact && 'py-4')}>
        <Card className="border border-white/10 bg-black/60 p-6">
          <AlertCircle className="w-12 h-12 text-slate-500 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-white mb-2">No significant moves found</h2>
          <p className="text-slate-400 mb-4">
            No significant price movements detected for {activeTicker} in this period.
          </p>
          {marketMode === 'ticker' && (
            <TickerSearch
              value={ticker}
  ...
  return (
    <div className={cn('space-y-6', compact && 'space-y-4')}>
      {/* Controls */}
      <div className={cn('flex items-center gap-2 flex-wrap', compact && 'gap-2')}>
        {marketMode === 'market' ? (
          <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-lg p-1">
            <Button
              variant={marketTicker === 'SPY' ? 'default' : 'ghost'}
              size="sm"
              className={cn(
                'h-7 px-3 text-xs',
                marketTicker === 'SPY'
              ...
              onClick={() => setMarketTicker('SPY')}
            >
              SPY
            </Button>
            <Button
              variant={marketTicker === 'QQQ' ? 'default' : 'ghost'}
              size="sm"
              className={cn(
                'h-7 px-3 text-xs',
                marketTicker === 'QQQ'
              ...
              onClick={() => setMarketTicker('QQQ')}
            >
              QQQ
            </Button>
          </div>
        ) : (
          <div className={cn('flex-1 min-w-[200px]', compact && 'min-w-[150px]')}>
            <TickerSearch
              value={ticker}
              onChange={setTicker}
              savedTickers={savedTickers}
        ...
        <DateRangePicker value={dateRange} onChange={setDateRange} className={compact ? 'text-xs' : ''} />
      ...
      {/* AI Explanation Toggle - only show if not compact */}
      {!compact && (
        <div className="flex items-center gap-3">
      ...
      {!compact ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Move Detail */}
          {!showDetailSheet && <MoveDetail moveWithCatalysts={selectedMoveWithCatalysts} />}
        </div>
      ) : (
        <Card className="border border-white/10 bg-black/60 p-6">
          <h2 className="text-base font-semibold text-white mb-4">Significant Moves</h2>
      ...
      {/* Saved Tickers */}
      {!compact && savedTickers.length > 0 && marketMode === 'ticker' && (
