...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({
      ...result,
      message: 'Dry-run only. Use POST with Authorization and ALPACA_AUTO_PAPER_ENABLED=true to submit paper orders.',
    });
  } catch (err) {
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!input.dryRun && !isAuthorized(req)) {
      return NextResponse.json({
        error: 'Unauthorized. Auto paper execution requires Authorization: Bearer EXECUTION_CRON_SECRET or CRON_SECRET.',
      }, { status: 401 });
    }
