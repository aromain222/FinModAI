...
import { fetchRecentMnaFilings } from '@/lib/secApi';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fetchRecentMnaFilings({
      ticker,
      limit,
      asOfDate,
      forms,
    ...
    if (!result) {
      return NextResponse.json({ error: `Unable to resolve SEC M&A filings for ${ticker}` }, { status: 404 });
    }
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch SEC M&A filings';
