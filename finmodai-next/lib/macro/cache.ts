...
const CACHE_PATH = path.join(process.cwd(), '.macro-cache.json');
...
export async function getMacroCache<T>(key: string): Promise<CacheEntry<T> | null> {
  const cache = await readCacheFile();
  return cache[key] ?? null;
}

export async function setMacroCache<T>(key: string, payload: T, updatedAt: string) {
  const cache = await readCacheFile();
