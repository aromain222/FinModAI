...
export async function recordPaperFill(input: RecordPaperFillInput): Promise<RecordPaperFillResult> {
  if (input.confidence < MIN_CONFIDENCE) {
    return { recorded: false, reason: `confidence_too_low (${input.confidence} < ${MIN_CONFIDENCE})` };
  }
  ...
  if (classified === 'buy') {
    ...
    const openOrders = await listPaperOrders({ ticker, limit: 200 });
    const positions = derivePaperPositions(openOrders);
