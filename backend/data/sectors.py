...
class SectorRegistry:
    ...
    def _load_default_mappings(self) -> None:
        ...
        # Technology
        tech_tickers = ['AAPL', 'MSFT', 'GOOGL', 'GOOG', 'META', 'NVDA', 'ORCL', 
                       'CSCO', 'INTC', 'AMD', 'CRM', 'ADBE', 'AVGO', 'TXN', 'QCOM']
        for t in tech_tickers:
            self.sector_map[t] = 'Technology'
        
        # Healthcare
        healthcare_tickers = ['UNH', 'JNJ', 'LLY', 'ABBV', 'PFE', 'MRK', 'TMO', 
                             'ABT', 'DHR', 'BMY', 'AMGN', 'CVS', 'CI']
        for t in healthcare_tickers:
            self.sector_map[t] = 'Healthcare'
        
        # Financial Services
        financial_tickers = ['JPM', 'BAC', 'WFC', 'GS', 'MS', 'BLK', 'C', 'USB', 
                            'PNC', 'TFC', 'COF', 'AXP', 'SCHW', 'BK', 'STT']
        for t in financial_tickers:
            self.sector_map[t] = 'Financial Services'
        
        # Consumer Discretionary
        consumer_disc_tickers = ['AMZN', 'TSLA', 'HD', 'MCD', 'NKE', 'LOW', 'SBUX', 
                                'TJX', 'BKNG', 'CMG', 'TGT', 'F', 'GM']
        ...
        # Industrials
        industrial_tickers = ['BA', 'UNP', 'HON', 'UPS', 'RTX', 'CAT', 'DE', 'LMT', 
                             'GE', 'MMM', 'GD', 'EMR']
        for t in industrial_tickers:
            self.sector_map[t] = 'Industrials'
        
        # Consumer Staples
        staples_tickers = ['WMT', 'PG', 'KO', 'PEP', 'COST', 'PM', 'MO', 'CL', 
                          'MDLZ', 'KMB', 'GIS', 'KHC']
        ...
        # Utilities
        utility_tickers = ['NEE', 'DUK', 'SO', 'D', 'AEP', 'EXC', 'SRE', 'XEL', 
                          'WEC', 'ED', 'ES', 'PEG']
        for t in utility_tickers:
            self.sector_map[t] = 'Utilities'
        
        # Real Estate
        realestate_tickers = ['AMT', 'PLD', 'CCI', 'EQIX', 'PSA', 'SPG', 'DLR', 
                             'O', 'WELL', 'AVB', 'EQR', 'VICI']
        for t in realestate_tickers:
            self.sector_map[t] = 'Real Estate'
        
        # Materials
        materials_tickers = ['LIN', 'APD', 'SHW', 'ECL', 'DD', 'NEM', 'FCX', 'DOW', 
                            'NUE', 'VMC', 'MLM']
