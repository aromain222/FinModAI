...
from backend.api.v1.models import (
    SnapshotResponse, SnapshotRow, LeadersResponse, AllSectorLeadersResponse,
    CompanyDetail, SortField, SortOrder
)
...
async def get_market_snapshot(
    ...
    limit: int = Query(config.DEFAULT_SNAPSHOT_LIMIT, ge=1, le=config.MAX_SNAPSHOT_LIMIT),
    ...
    if not all_snapshots:
        raise HTTPException(status_code=503, detail="Market data not available yet. Try again in a moment.")
    ...
    snapshots_list.sort(
        key=lambda x: x.get(sort_field) if x.get(sort_field) is not None else (float('-inf') if reverse else float('inf')),
        reverse=reverse
    )
...
async def get_sector_leaders(
    ...
    if sector:
        ...
        return LeadersResponse(
            sector=sector,
            leaders=[SnapshotRow(**l) for l in leaders],
            limit=limit
        )
        ...
        for sec in GICS_SECTORS:
            ...
            if leaders:
                all_leaders.append(
                    LeadersResponse(
                        sector=sec,
                        leaders=[SnapshotRow(**l) for l in leaders[:limit]],
                        limit=limit
                    )
                )
